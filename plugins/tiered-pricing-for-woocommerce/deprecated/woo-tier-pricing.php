						...
						foreach ( $tier_data as $product_id => $data ) {
							...
							if ( $product_data ) {
								...
								if ( ! ( 'variation' === $product_data->get_type() ) ) {
									...
									if ( ! empty( $data ) ) {
										$product_data->update_meta_data( 'wtp_product_tier_setting', $data );
										echo 'success';
									} else {
										echo '';
									}
										...
										$product_data->update_meta_data( 'wtp_product_tier_setting_' . $product_id, $data );
										echo 'success';
								} else {
									echo '';
					...
					if ( is_array( $products ) && count( $products ) > 0 ) {
						...
						foreach ( $products as $ID ) {
							...
							if ( 'variation' == $product->get_type() ) {
								$temp_data = $product->get_meta( 'wtp_product_tier_setting_' . $ID, true );
								if ( isset( $temp_data['min_qty'] ) ) {
									...
									if ( empty( $check ) ) {
										$product->update_meta_data( 'wtp_product_tier_set', true );
									}
								}
							}

							if ( 'simple' == $product->get_type() ) {
								$temp_data = $product->get_meta( 'wtp_product_tier_setting', true );
								if ( isset( $temp_data['min_qty'] ) ) {
									foreach ( $temp_data['min_qty'] as $k => $v ) {
										array_push(
											$data,
						...
						**/
						...
						if ( ! file_exists( $filepath ) ) {
							mkdir( $filepath, 0777, true );
								...
								unlink( $filepath . $file_name ); // nosemgrep audit.php.lang.security.file.read-write-delete
						}
			...
			if (is_object($product_data)) {
				...
				if ( ! ( $product_data->is_type( 'variable' ) || $product_data->is_type( 'variable-subscription' ) ) ) {
					if ( isset( $_POST['wtp_product_tier_setting'] ) ) {
						...
						$data = $_POST;
						$product_data->update_meta_data( 'wtp_product_tier_setting', $data['wtp_product_tier_setting'] );
						$product_data->update_meta_data( 'wtp_product_tier_set', true );
					} elseif ( 'inline-save' == $data['action'] ) {
						return;
					} else {
						$product_data->update_meta_data( 'wtp_product_tier_setting', '' );
						$product_data->update_meta_data( 'wtp_product_tier_set', false );
					}
				}
				$product_data->save();
			}
			...
			if ( isset( $data['variable_post_id'] ) ) {
				...
				foreach ( $variable_post_id as $key => $variation_id ) {
					...
					if ( isset( $data[ 'wtp_product_tier_setting_' . $variation_id ] ) ) {
						$variation->update_meta_data( 'wtp_product_tier_setting_' . $variation_id, $data[ 'wtp_product_tier_setting_' . $variation_id ] );
						$variation->update_meta_data( 'wtp_product_tier_set', true );
					} else {
						$variation->update_meta_data( 'wtp_product_tier_setting_' . $variation_id, '' );
						$variation->update_meta_data( 'wtp_product_tier_set', false );
					}
					$variation->save();
				}
			}
