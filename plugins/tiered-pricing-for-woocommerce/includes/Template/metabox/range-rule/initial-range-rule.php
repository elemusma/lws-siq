<?php
if ( empty( $wtp_tier_clone ) ) {
	$wtp_tier_clone = array(
	   array(
		   'wtp_min_qty' => '1',
		   'wtp_max_qty' => '2',
		   'wtp_discount_type' => '',
		   'wtp_discount_value' => '0',
	   ),
	);
}
?>
<div class="tier-range-row">
	<div class="tier-form-group">
		<input class="minQty" type="number" min="1" oninput="this.value = Math.abs(this.value)" name="wtp_tier_clone[wtp_min_qty][]" value="<?php esc_attr_e( $wtp_tier_clone[0]['wtp_min_qty'] ); ?>">
		<label><?php esc_html_e( 'Minimum Quantity', 'wtp' ); ?></label>
	</div>
	
	<div class="tier-form-group">
		<input class="maxQty" type="number" min="1" oninput="this.value = Math.abs(this.value)" name="wtp_tier_clone[wtp_max_qty][]" value="<?php esc_attr_e( $wtp_tier_clone[0]['wtp_max_qty'] ); ?>">
		<label><?php esc_html_e( 'Maximum Quantity', 'wtp' ); ?></label>
	</div>
	
	<div class="tier-form-group">
		<select name="wtp_tier_clone[wtp_discount_type][]">
			<option <?php selected( 'fix', $wtp_tier_clone[0]['wtp_discount_type'], true ); ?> value="fix">Fixed</option>
			<option <?php selected( 'percent', $wtp_tier_clone[0]['wtp_discount_type'], true ); ?> value="percent">Percentage</option>
		</select>
		<label><?php esc_html_e( 'Discount Type', 'wtp' ); ?></label>
	</div>
	
	<div class="tier-form-group">
		<input class="discount-value" type="number" step="any" min="0.01" name="wtp_tier_clone[wtp_discount_value][]" value="<?php esc_attr_e( $wtp_tier_clone[0]['wtp_discount_value'] ); ?>">
