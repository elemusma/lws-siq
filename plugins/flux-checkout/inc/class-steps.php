...
class Iconic_Flux_Steps {
	...
	public static function render_payment_details() {
		...
		if ( $is_modern_theme ) {
			self::render_customer_review();
			?>
			<h2 class="flux-heading flux-heading--payment"><?php echo esc_html__( 'Payment', 'flux-checkout' ); ?></h2>
			<?php
		}
			...
			<div class="billing-address-search<?php echo $is_pre_populated ? ' billing-address-search--pre-populated' : ''; ?>">
				...
				<p class="form-row form-row-wide is-active" id="billing_address_info">
					...
					<span class="woocommerce-input-wrapper">
						<input type="text" class="input-text" name="billing_address_search" id="billing_address_search" value="" placeholder="<?php echo esc_attr( __( 'Enter a location', 'flux-checkout' ) ); ?>"/>
					</span>
		...
		if ( ( $is_modern && Iconic_Flux_Helpers::use_autocomplete() ) || ( Iconic_Flux_Helpers::use_autocomplete() && ! $is_pre_populated ) ) {
			...
			<div class="shipping-address-search<?php echo $is_pre_populated ? ' shipping-address-search--pre-populated' : ''; ?>">
				...
				<p class="form-row form-row-wide" id="shipping_address_info">
					...
					<span class="woocommerce-input-wrapper">
						<input type="text" class="input-text" name="shipping_address_search" id="shipping_address_search" value="" placeholder="<?php echo esc_attr( __( 'Enter a location', 'flux-checkout' ) ); ?>" />
					</span>
				</p>
	...
	public static function get_customer_details() {
		...
		if ( $packages && isset( $packages[0] ) ) {
			...
			$packages[0]['destination']['address_1'] = ! empty( $posted_data['ship_to_different_address'] ) ? Iconic_Flux_Helpers::rgar( $posted_data, 'shipping_address_1' ) : Iconic_Flux_Helpers::rgar( $posted_data, 'billing_address_1' );
			$packages[0]['destination']['city']      = ! empty( $posted_data['ship_to_different_address'] ) ? Iconic_Flux_Helpers::rgar( $posted_data, 'shipping_city' ) : Iconic_Flux_Helpers::rgar( $posted_data, 'billing_city' );
			$packages[0]['destination']['postcode']  = ! empty( $posted_data['ship_to_different_address'] ) ? Iconic_Flux_Helpers::rgar( $posted_data, 'shipping_postcode' ) : Iconic_Flux_Helpers::rgar( $posted_data, 'billing_postcode' );
			$packages[0]['destination']['state']     = ! empty( $posted_data['ship_to_different_address'] ) ? Iconic_Flux_Helpers::rgar( $posted_data, 'shipping_state' ) : Iconic_Flux_Helpers::rgar( $posted_data, 'billing_state' );
			$packages[0]['destination']['country']   = ! empty( $posted_data['ship_to_different_address'] ) ? Iconic_Flux_Helpers::rgar( $posted_data, 'shipping_country' ) : Iconic_Flux_Helpers::rgar( $posted_data, 'billing_country' );
			...
			if ( '1' === Iconic_Flux_Core_Settings::$settings['general_general_separate_street_number_field'] ) {
				...
				$data['stret_number'] = $street_number;
