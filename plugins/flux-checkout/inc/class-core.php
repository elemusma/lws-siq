...
class Iconic_Flux_Core {
	...
	public static function custom_override_checkout_fields( $fields ) {
		...
		} elseif ( 'required' === self::phone_field_status() ) {
	...
	public static function company_field_status() {
		...
		$block_attribute = Iconic_Flux_Compat_Checkout_Block::get_checkout_attributes();

		if ( empty( $block_attribute ) || ! array_key_exists( 'showCompanyField', $block_attribute ) ) {
			if ( ! $settings['general_general_show_company_field'] ) {
				return 'hidden';
			} else {
				// In WC 8.8, WooCommerce Customizer setting is removed in favour of block attribute.
				if ( version_compare( WC()->version, '8.7', '<=' ) ) {
					return get_option( 'woocommerce_checkout_company_field', 'optional' );
				} else {
					return 'optional';
				}
			}
		...
		$return = $block_attribute['requireCompanyField'] ? 'required' : 'optional';

		return $block_attribute['showCompanyField'] ? $return : 'hidden';
	}

	/**
	 * Is company field enabled.
	 *
	 * @return string 'required', 'option' or 'hidden'
	 */
	public static function phone_field_status() {
		...
		if ( empty( $block_attribute ) || ! array_key_exists( 'showPhoneField', $block_attribute ) ) {
			// In WC 8.8, WooCommerce Customizer setting is removed in favour of block attribute.
			if ( version_compare( WC()->version, '8.7', '<=' ) ) {
		...
		$return = $block_attribute['requirePhoneField'] ? 'required' : 'optional';

		return $block_attribute['showPhoneField'] ? $return : 'hidden';
	}
}
